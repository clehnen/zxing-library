...
var HTMLCanvasElementLuminanceSource = require('./HTMLCanvasElementLuminanceSource');
var VideoInputDevice = require('./VideoInputDevice');
...
class BrowserCodeReader {
   ...
   */
  _stopContinuousDecode = false;
  /**
   * This will break the loop.
   */
  _stopAsyncDecode = false;
  /**
   * Delay time between decode attempts made by the scanner.
   */
  _timeBetweenDecodingAttempts = 0;
  ...
  async getVideoInputDevices() {
    ...
    return devices.map((d) => new VideoInputDevice.VideoInputDevice(d.deviceId, d.label));
  }
  /**
   ...
   */
  async decodeFromInputVideoDevice(deviceId, videoSource) {
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeOnceFromVideoDevice(deviceId, videoSource) {
    this.reset();
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeOnceFromConstraints(constraints, videoSource) {
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeOnceFromStream(stream, videoSource) {
    this.reset();
   ...
   */
  async decodeFromInputVideoDeviceContinuously(deviceId, videoSource, callbackFn) {
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeFromVideoDevice(deviceId, videoSource, callbackFn) {
    let videoConstraints;
    if (!deviceId) {
      videoConstraints = { facingMode: "environment" };
    } else {
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeFromConstraints(constraints, videoSource, callbackFn) {
    const stream = await navigator.mediaDevices.getUserMedia(constraints);
   ...
   * @memberOf BrowserCodeReader
   */
  async decodeFromStream(stream, videoSource, callbackFn) {
    this.reset();
    const video = await this.attachStreamToVideo(stream, videoSource);
  ...
  playVideoOnLoadAsync(videoElement) {
    return new Promise(
      (resolve, reject) => this.playVideoOnLoad(videoElement, () => resolve())
    );
  }
  /**
  ...
  decodeFromImageElement(source) {
    if (!source) {
      throw new ArgumentException.ArgumentException("An image element must be provided.");
    }
    this.reset();
  ...
  _decodeFromVideoElementSetup(source) {
    if (!source) {
      throw new ArgumentException.ArgumentException("A video element must be provided.");
    }
    this.reset();
  ...
  decodeFromImageUrl(url) {
    if (!url) {
      throw new ArgumentException.ArgumentException("An URL must be provided.");
    }
    this.reset();
  ...
  async _decodeOnLoadVideo(videoElement) {
    await this.playVideoOnLoadAsync(videoElement);
    return await this.decodeOnce(videoElement);
  }
  async _decodeOnLoadVideoContinuously(videoElement, callbackFn) {
    await this.playVideoOnLoadAsync(videoElement);
  ...
  decodeOnce(element, retryIfNotFound = true, retryIfChecksumOrFormatError = true) {
    ...
    const loop = (resolve, reject) => {
      ...
      try {
        ...
        const ifNotFound = retryIfNotFound && e instanceof NotFoundException.NotFoundException;
        const isChecksumOrFormatError = e instanceof ChecksumException.ChecksumException || e instanceof FormatException.FormatException;
  ...
  decodeContinuously(element, callbackFn) {
    ...
    const loop = () => {
      ...
      try {
        ...
        callbackFn(null, e);
        const isChecksumOrFormatError = e instanceof ChecksumException.ChecksumException || e instanceof FormatException.FormatException;
        const isNotFound = e instanceof NotFoundException.NotFoundException;
        if (isChecksumOrFormatError || isNotFound) {
          setTimeout(loop, this._timeBetweenDecodingAttempts);
        } else {
          console.error(
            "BrowserCodeReader: unexpected error during decode, stopping scan loop:",
            e
          );
        }
      }
  ...
  decode(element) {
    ...
    try {
      ...
      if (e instanceof NotFoundException.NotFoundException) {
        ...
        const luminanceSource = new HTMLCanvasElementLuminanceSource.HTMLCanvasElementLuminanceSource(
          canvas,
          false
        );
        const globalBinarizer = new GlobalHistogramBinarizer.GlobalHistogramBinarizer(luminanceSource);
        const fallbackBitmap = new BinaryBitmap.BinaryBitmap(globalBinarizer);
  ...
  createBinaryBitmap(mediaElement) {
    ...
    const luminanceSource = new HTMLCanvasElementLuminanceSource.HTMLCanvasElementLuminanceSource(
      canvas,
      doAutoInvert,
      this._grayscaleBuffer
    );
    this._grayscaleBuffer = luminanceSource.getMatrix();
    const hybridBinarizer = new HybridBinarizer.HybridBinarizer(luminanceSource);
    return new BinaryBitmap.BinaryBitmap(hybridBinarizer);
  }
  /**
   *
   */
...
//# sourceMappingURL=BrowserCodeReader.cjs.map
//# sourceMappingURL=BrowserCodeReader.cjs.map