...
var InvertedLuminanceSource = require('./InvertedLuminanceSource');
var IllegalArgumentException = require('./IllegalArgumentException');

class PlanarYUVLuminanceSource extends LuminanceSource.LuminanceSource {
  constructor(yuvData, dataWidth, dataHeight, left, top, width, height, reverseHorizontal) {
    ...
    if (left + width > dataWidth || top + height > dataHeight) {
      throw new IllegalArgumentException.IllegalArgumentException("Crop rectangle does not fit within image data.");
    }
  ...
  getRow(y, row) {
    ...
    ZXingSystem.ZXingSystem.arraycopy(this.yuvData, offset, row, 0, width);
    return row;
  }
  /*@Override*/
  getMatrix() {
    ...
    if (width === this.dataWidth) {
      ZXingSystem.ZXingSystem.arraycopy(this.yuvData, inputOffset, matrix, 0, area);
      return matrix;
    }
    for (let y = 0; y < height; y++) {
      const outputOffset = y * width;
      ZXingSystem.ZXingSystem.arraycopy(this.yuvData, inputOffset, matrix, outputOffset, width);
...
//# sourceMappingURL=PlanarYUVLuminanceSource.cjs.map
//# sourceMappingURL=PlanarYUVLuminanceSource.cjs.map