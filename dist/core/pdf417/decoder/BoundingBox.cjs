...
class BoundingBox {
  ...
  constructor_1(image, topLeft, bottomLeft, topRight, bottomRight) {
    ...
    if (leftUnspecified) {
      topLeft = new ResultPoint.ResultPoint(0, topRight.getY());
      bottomLeft = new ResultPoint.ResultPoint(0, bottomRight.getY());
    } else if (rightUnspecified) {
      topRight = new ResultPoint.ResultPoint(image.getWidth() - 1, topLeft.getY());
      bottomRight = new ResultPoint.ResultPoint(image.getWidth() - 1, bottomLeft.getY());
    }
    this.image = image;
  ...
  addMissingRows(missingStartRows, missingEndRows, isLeft) {
    ...
    if (missingStartRows > 0) {
      ...
      let newTop = new ResultPoint.ResultPoint(top.getX(), newMinY);
      if (isLeft) {
        newTopLeft = newTop;
      } else {
        newTopRight = newTop;
    ...
    if (missingEndRows > 0) {
      ...
      let newBottom = new ResultPoint.ResultPoint(bottom.getX(), newMaxY);
      if (isLeft) {
...
//# sourceMappingURL=BoundingBox.cjs.map
//# sourceMappingURL=BoundingBox.cjs.map