...
var PDF417DecoderErrorCorrection = require('./ec/PDF417DecoderErrorCorrection');
var BoundingBox = require('./BoundingBox');
var DetectionResultRowIndicatorColumn = require('./DetectionResultRowIndicatorColumn');
var DetectionResult = require('./DetectionResult');
var DetectionResultColumn = require('./DetectionResultColumn');
var Codeword = require('./Codeword');
var BarcodeValue = require('./BarcodeValue');
var PDF417CodewordDecoder = require('./PDF417CodewordDecoder');
var PDF417DecodedBitStreamParser = require('./PDF417DecodedBitStreamParser');
...
class PDF417ScanningDecoder {
  ...
  /*final*/
  static errorCorrection = new PDF417DecoderErrorCorrection.PDF417DecoderErrorCorrection();
  constructor() {
  }
  /**
  ...
  static decode(image, imageTopLeft, imageBottomLeft, imageTopRight, imageBottomRight, minCodewordWidth, maxCodewordWidth) {
    let boundingBox = new BoundingBox.BoundingBox(image, imageTopLeft, imageBottomLeft, imageTopRight, imageBottomRight);
    ...
    for (let barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
      ...
      if (barcodeColumn === 0 || barcodeColumn === maxBarcodeColumn) {
        detectionResultColumn = new DetectionResultRowIndicatorColumn.DetectionResultRowIndicatorColumn(boundingBox, barcodeColumn === 0);
      } else {
        detectionResultColumn = new DetectionResultColumn.DetectionResultColumn(boundingBox);
      }
  ...
  static merge(leftRowIndicatorColumn, rightRowIndicatorColumn) {
    ...
    return new DetectionResult.DetectionResult(barcodeMetadata, boundingBox);
  }
  /**
   *
  ...
  static getRowIndicatorColumn(image, boundingBox, startPoint, leftToRight, minCodewordWidth, maxCodewordWidth) {
    let rowIndicatorColumn = new DetectionResultRowIndicatorColumn.DetectionResultRowIndicatorColumn(
      boundingBox,
      leftToRight
    );
    for (let i = 0; i < 2; i++) {
  ...
  static adjustCodewordCount(detectionResult, barcodeMatrix) {
    ...
    if (numberOfCodewords.length === 0) {
      if (calculatedNumberOfCodewords < 1 || calculatedNumberOfCodewords > PDF417Common.PDF417Common.MAX_CODEWORDS_IN_BARCODE) {
        throw NotFoundException.NotFoundException.getNotFoundInstance();
      }
  ...
  static createDecoderResult(detectionResult) {
    ...
    return PDF417ScanningDecoder.createDecoderResultFromAmbiguousValues(
      ...
      codewords,
      PDF417Common.PDF417Common.toIntArray(erasures),
      PDF417Common.PDF417Common.toIntArray(ambiguousIndexesList),
  ...
  static createDecoderResultFromAmbiguousValues(ecLevel, codewords, erasureArray, ambiguousIndexes, ambiguousIndexValues) {
    ...
    while (tries-- > 0) {
      ...
      try {
        ...
        let ignored = err instanceof ChecksumException.ChecksumException;
        if (!ignored) {
          throw err;
        }
      }
      if (ambiguousIndexCount.length === 0) {
        throw ChecksumException.ChecksumException.getChecksumInstance();
      }
      for (let i = 0; i < ambiguousIndexCount.length; i++) {
  ...
  static createBarcodeMatrix(detectionResult) {
    ...
    for (let row = 0; row < barcodeMatrix.length; row++) {
      for (let column2 = 0; column2 < barcodeMatrix[row].length; column2++) {
        barcodeMatrix[row][column2] = new BarcodeValue.BarcodeValue();
      }
    }
    let column = 0;
  ...
  static detectCodeword(image, minColumn, maxColumn, leftToRight, startColumn, imageRow, minCodewordWidth, maxCodewordWidth) {
    ...
    let endColumn;
    let codewordBitCount = MathUtils.MathUtils.sum(moduleBitCount);
    if (leftToRight) {
    ...
    let decodedValue = PDF417CodewordDecoder.PDF417CodewordDecoder.getDecodedValue(moduleBitCount);
    let codeword = PDF417Common.PDF417Common.getCodeword(decodedValue);
    if (codeword === -1) {
      return null;
    }
    return new Codeword.Codeword(startColumn, endColumn, PDF417ScanningDecoder.getCodewordBucketNumber(decodedValue), codeword);
  }
  ...
  static decodeCodewords(codewords, ecLevel, erasures) {
    ...
    let decoderResult = PDF417DecodedBitStreamParser.PDF417DecodedBitStreamParser.decode(codewords, "" + ecLevel);
...
exports.PDF417ScanningDecoder = PDF417ScanningDecoder;
//# sourceMappingURL=PDF417ScanningDecoder.cjs.map
//# sourceMappingURL=PDF417ScanningDecoder.cjs.map