...
var PDF417DetectorResult = require('./PDF417DetectorResult');
...
class Detector {
  ...
  static detectMultiple(image, hints, multiple) {
    ...
    return new PDF417DetectorResult.PDF417DetectorResult(bitMatrix, barcodeCoordinates);
  }
  /**
  ...
  static findRowsWithPattern(matrix, height, width, startRow, startColumn, pattern) {
    ...
    for (; startRow < height; startRow += Detector.ROW_STEP) {
      ...
      if (loc != null) {
        ...
        result[0] = new ResultPoint.ResultPoint(loc[0], startRow);
        result[1] = new ResultPoint.ResultPoint(loc[1], startRow);
        found = true;
        break;
      }
    }
    ...
    if (found) {
      ...
      result[2] = new ResultPoint.ResultPoint(previousRowLoc[0], stopRow);
      result[3] = new ResultPoint.ResultPoint(previousRowLoc[1], stopRow);
    }
    if (stopRow - startRow < Detector.BARCODE_MIN_HEIGHT) {
      ZXingArrays.ZXingArrays.fill(result, null);
    }
    return result;
  }
  /**
  ...
  static findGuardPattern(matrix, column, row, width, whiteFirst, pattern, counters) {
    ZXingArrays.ZXingArrays.fillWithin(counters, 0, counters.length, 0);
