...
var GenericGF = require('./GenericGF');
var GenericGFPoly = require('./GenericGFPoly');
var ReedSolomonException = require('../../ReedSolomonException');
...
class ReedSolomonDecoder {
  ...
  decodeWithECCount(received, twoS) {
    ...
    const poly = new GenericGFPoly.GenericGFPoly(field, received);
    ...
    for (let i = 0; i < errorLocations.length; i++) {
      ...
      if (position < 0) {
        throw new ReedSolomonException.ReedSolomonException("Bad error location");
      }
      received[position] = GenericGF.GenericGF.addOrSubtract(received[position], errorMagnitudes[i]);
    }
  ...
  runEuclideanAlgorithm(a, b, R) {
    ...
    while (r.getDegree() >= (R / 2 | 0)) {
      ...
      tLast = t;
      if (rLast.isZero()) {
        throw new ReedSolomonException.ReedSolomonException("r_{i-1} was zero");
      }
      r = rLastLast;
      let q = field.getZero();
      ...
      if (r.getDegree() >= rLast.getDegree()) {
        throw new IllegalStateException.IllegalStateException("Division algorithm failed to reduce polynomial?");
      }
    }
    const sigmaTildeAtZero = t.getCoefficient(0);
    if (sigmaTildeAtZero === 0) {
      throw new ReedSolomonException.ReedSolomonException("sigmaTilde(0) was zero");
    }
  ...
  findErrorLocations(errorLocator) {
    ...
    if (e !== numErrors) {
      throw new ReedSolomonException.ReedSolomonException("Error locator degree does not match number of roots");
    }
    return result;
  }
...
//# sourceMappingURL=ReedSolomonDecoder.cjs.map
//# sourceMappingURL=ReedSolomonDecoder.cjs.map