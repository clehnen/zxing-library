...
class BitArray {
  ...
  ensureCapacity(size) {
    if (size > this.bits.length * 32) {
      ...
      ZXingSystem.ZXingSystem.arraycopy(this.bits, 0, newBits, 0, this.bits.length);
      this.bits = newBits;
    }
  }
  /**
  ...
  getNextSet(from) {
    ...
    const result = bitsOffset * 32 + ZXingInteger.ZXingInteger.numberOfTrailingZeros(currentBits);
  ...
  appendBits(value, numBits) {
    if (numBits < 0 || numBits > 32) {
      throw new IllegalArgumentException.IllegalArgumentException("Num bits must be between 0 and 32");
    }
  ...
  xor(other) {
    if (this.size !== other.size) {
      throw new IllegalArgumentException.IllegalArgumentException("Sizes don't match");
    }
    const bits = this.bits;
  ...
  equals(o) {
    ...
    const other = o;
    return this.size === other.size && ZXingArrays.ZXingArrays.equals(this.bits, other.bits);
  }
  /*@Override*/
  hashCode() {
    return 31 * this.size + ZXingArrays.ZXingArrays.hashCode(this.bits);
  }
  /*@Override*/
  toString() {
    let result = "";
