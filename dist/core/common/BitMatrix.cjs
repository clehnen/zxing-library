...
class BitMatrix {
  ...
  constructor(width, height, rowSize, bits) {
    ...
    this.height = height;
    if (width < 1 || height < 1) {
      throw new IllegalArgumentException.IllegalArgumentException("Both dimensions must be greater than 0");
    }
  ...
  static parseFromString(stringRepresentation, setString, unsetString) {
    if (stringRepresentation === null) {
      throw new IllegalArgumentException.IllegalArgumentException("stringRepresentation cannot be null");
    }
      ...
      if (stringRepresentation.charAt(pos) === "\n" || stringRepresentation.charAt(pos) === "\r") {
        if (bitsPos > rowStartPos) {
          if (rowLength === -1) {
            ...
            throw new IllegalArgumentException.IllegalArgumentException("row lengths do not match");
          }
          rowStartPos = bitsPos;
          nRows++;
        }
  ...
  xor(mask) {
    if (this.width !== mask.getWidth() || this.height !== mask.getHeight() || this.rowSize !== mask.getRowSize()) {
      throw new IllegalArgumentException.IllegalArgumentException("input matrix dimensions do not match");
    }
    const rowArray = new BitArray.BitArray((this.width >> 5) + 1);
  ...
  setRegion(left, top, width, height) {
    if (top < 0 || left < 0) {
      throw new IllegalArgumentException.IllegalArgumentException("Left and top must be nonnegative");
    }
    if (height < 1 || width < 1) {
      throw new IllegalArgumentException.IllegalArgumentException("Height and width must be at least 1");
    }
    const right = left + width;
    const bottom = top + height;
    if (bottom > this.height || right > this.width) {
      throw new IllegalArgumentException.IllegalArgumentException("The region must fit inside the matrix");
    }
    const rowSize = this.rowSize;
    const bits = this.bits;
  ...
  getRow(y, row) {
    if (row === null || row === void 0 || row.getSize() < this.width) {
      row = new BitArray.BitArray(this.width);
    } else {
      row.clear();
    }
    const rowSize = this.rowSize;
   ...
   */
  setRow(y, row) {
    ZXingSystem.ZXingSystem.arraycopy(row.getBitArray(), 0, this.bits, y * this.rowSize, this.rowSize);
  }
  /**
  ...
  rotate180() {
    ...
    let topRow = new BitArray.BitArray(width);
    let bottomRow = new BitArray.BitArray(width);
  ...
  equals(o) {
    ...
    const other = o;
    return this.width === other.width && this.height === other.height && this.rowSize === other.rowSize && ZXingArrays.ZXingArrays.equals(this.bits, other.bits);
  }
  /*@Override*/
  hashCode() {
    let hash = this.width;
    ...
    hash = 31 * hash + ZXingArrays.ZXingArrays.hashCode(this.bits);
    return hash;
  }
  /**
...
//# sourceMappingURL=BitMatrix.cjs.map
//# sourceMappingURL=BitMatrix.cjs.map