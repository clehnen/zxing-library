...
class MicroQRDecodedBitStreamParser {
  ...
  static decode(bytes, version, hints) {
    ...
    try {
      ...
      if (e instanceof FormatException.FormatException) throw e;
      throw new FormatException.FormatException();
    }
  ...
  static decodeNumeric(bits, result, count) {
    let remaining = count;
    while (remaining >= 3) {
      if (bits.available() < 10) throw new FormatException.FormatException();
      const threeDigits = bits.readBits(10);
      if (threeDigits >= 1e3) throw new FormatException.FormatException();
    ...
    if (remaining === 2) {
      if (bits.available() < 7) throw new FormatException.FormatException();
      const twoDigits = bits.readBits(7);
      if (twoDigits >= 100) throw new FormatException.FormatException();
      ...
      if (bits.available() < 4) throw new FormatException.FormatException();
      const oneDigit = bits.readBits(4);
      if (oneDigit >= 10) throw new FormatException.FormatException();
  ...
  static decodeAlphanumeric(bits, result, count) {
    let remaining = count;
    while (remaining > 1) {
      if (bits.available() < 11) throw new FormatException.FormatException();
    ...
    if (remaining === 1) {
      if (bits.available() < 6) throw new FormatException.FormatException();
  ...
  static decodeByte(bits, result, byteSegments, count, hints) {
    if (8 * count > bits.available()) throw new FormatException.FormatException();
    const readBytes = new Uint8Array(count);
    for (let i = 0; i < count; i++) {
      readBytes[i] = bits.readBits(8) & 255;
    }
    const encoding = StringUtils.StringUtils.guessEncoding(readBytes, hints);
    try {
  ...
  static decodeKanji(bits, result, count) {
    if (13 * count > bits.available()) throw new FormatException.FormatException();
...
exports.MicroQRDecodedBitStreamParser = MicroQRDecodedBitStreamParser;
//# sourceMappingURL=MicroQRDecodedBitStreamParser.cjs.map
//# sourceMappingURL=MicroQRDecodedBitStreamParser.cjs.map