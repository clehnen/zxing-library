...
class AztecEncoder {
  ...
  static encode(data, minECCPercent, userSpecifiedLayers) {
    let bits = new AztecHighLevelEncoder.AztecHighLevelEncoder(data).encode();
    let eccBits = ZXingInteger.ZXingInteger.truncDivision(bits.getSize() * minECCPercent, 100) + 11;
    ...
    if (userSpecifiedLayers !== AztecEncoder.DEFAULT_AZTEC_LAYERS) {
      ...
      if (layers > (compact ? AztecEncoder.MAX_NB_BITS_COMPACT : AztecEncoder.MAX_NB_BITS)) {
        throw new IllegalArgumentException.IllegalArgumentException(
          StringUtils.StringUtils.format("Illegal value %s for layers", userSpecifiedLayers)
        );
      }
      ...
      if (stuffedBits.getSize() + eccBits > usableBitsInLayers) {
        throw new IllegalArgumentException.IllegalArgumentException("Data to large for user specified layer");
      }
      ...
      for (let i = 0; ; i++) {
        if (i > AztecEncoder.MAX_NB_BITS) {
          throw new IllegalArgumentException.IllegalArgumentException("Data too large for an Aztec code");
        }
        compact = i <= 3;
    ...
    if (compact) {
      ...
      matrixSize = baseMatrixSize + 1 + 2 * ZXingInteger.ZXingInteger.truncDivision(ZXingInteger.ZXingInteger.truncDivision(baseMatrixSize, 2) - 1, 15);
      let origCenter = ZXingInteger.ZXingInteger.truncDivision(baseMatrixSize, 2);
    ...
    let matrix = new BitMatrix.BitMatrix(matrixSize);
    ...
    if (compact) {
      AztecEncoder.drawBullsEye(matrix, ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2), 5);
    } else {
      AztecEncoder.drawBullsEye(matrix, ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2), 7);
      for (let i = 0, j = 0; i < ZXingInteger.ZXingInteger.truncDivision(baseMatrixSize, 2) - 1; i += 15, j += 16) {
        for (let k = ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2) & 1; k < matrixSize; k += 2) {
          matrix.set(ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2) - j, k);
          matrix.set(ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2) + j, k);
          matrix.set(k, ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2) - j);
          matrix.set(k, ZXingInteger.ZXingInteger.truncDivision(matrixSize, 2) + j);
        }
      }
    }
    let aztec = new AztecCode.AztecCode();
  ...
  static generateModeMessage(compact, layers, messageSizeInWords) {
    let modeMessage = new BitArray.BitArray();
    if (compact) {
  ...
  static drawModeMessage(matrix, compact, matrixSize, modeMessage) {
    ...
    if (compact) {
      ...
      for (let i = 0; i < 10; i++) {
        let offset = center - 5 + i + ZXingInteger.ZXingInteger.truncDivision(i, 5);
        if (modeMessage.get(i)) {
  ...
  static generateCheckWords(bitArray, totalBits, wordSize) {
    ...
    let rs = new ReedSolomonEncoder.ReedSolomonEncoder(AztecEncoder.getGF(wordSize));
    let totalWords = ZXingInteger.ZXingInteger.truncDivision(totalBits, wordSize);
    let messageWords = AztecEncoder.bitsToWords(bitArray, wordSize, totalWords);
    rs.encode(messageWords, totalWords - messageSizeInWords);
    let startPad = totalBits % wordSize;
    let messageBits = new BitArray.BitArray();
  ...
  static getGF(wordSize) {
    switch (wordSize) {
      case 4:
        return GenericGF.GenericGF.AZTEC_PARAM;
      case 6:
        return GenericGF.GenericGF.AZTEC_DATA_6;
      case 8:
        return GenericGF.GenericGF.AZTEC_DATA_8;
      case 10:
        return GenericGF.GenericGF.AZTEC_DATA_10;
      case 12:
        return GenericGF.GenericGF.AZTEC_DATA_12;
      default:
        throw new IllegalArgumentException.IllegalArgumentException("Unsupported word size " + wordSize);
    }
  }
  static stuffBits(bits, wordSize) {
    let out = new BitArray.BitArray();
    let n = bits.getSize();
...
exports.AztecEncoder = AztecEncoder;
//# sourceMappingURL=AztecEncoder.cjs.map
//# sourceMappingURL=AztecEncoder.cjs.map