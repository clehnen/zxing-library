...
class QRCodeMatrixUtil {
  ...
  static embedTypeInfo(ecLevel, maskPattern, matrix) {
    const typeInfoBits = new BitArray.BitArray();
  ...
  static embedDataBits(dataBits, maskPattern, matrix) {
    ...
    while (x > 0) {
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (let i = 0; i < 2; ++i) {
          ...
          if (maskPattern !== 255 && QRCodeMaskUtil.QRCodeMaskUtil.getDataMaskBit(maskPattern, xx, y)) {
            bit = !bit;
          }
    ...
    if (bitIndex !== dataBits.getSize()) {
      throw new WriterException.WriterException("Not all bits consumed: " + bitIndex + "/" + dataBits.getSize());
    }
  }
  ...
  static findMSBSet(value) {
    return 32 - ZXingInteger.ZXingInteger.numberOfLeadingZeros(value);
  }
  ...
  static calculateBCHCode(value, poly) {
    if (poly === 0) {
      throw new IllegalArgumentException.IllegalArgumentException("0 polynomial");
    }
  ...
  static makeTypeInfoBits(ecLevel, maskPattern, bits) {
    if (!QRCodeEncoderQRCode.QRCodeEncoderQRCode.isValidMaskPattern(maskPattern)) {
      throw new WriterException.WriterException("Invalid mask pattern");
    }
    ...
    bits.xor(maskBits);
    if (bits.getSize() !== 15) {
      throw new WriterException.WriterException("should not happen but we got: " + bits.getSize());
    }
  }
...
exports.QRCodeMatrixUtil = QRCodeMatrixUtil;
//# sourceMappingURL=QRCodeMatrixUtil.cjs.map
//# sourceMappingURL=QRCodeMatrixUtil.cjs.map