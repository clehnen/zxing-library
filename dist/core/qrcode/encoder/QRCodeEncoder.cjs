...
var ReedSolomonEncoder = require('../../common/reedsolomon/ReedSolomonEncoder');
var QRCodeMode = require('../decoder/QRCodeMode');
var QRCodeVersion = require('../decoder/QRCodeVersion');
var QRCodeMaskUtil = require('./QRCodeMaskUtil');
var QRCodeByteMatrix = require('./QRCodeByteMatrix');
var QRCodeEncoderQRCode = require('./QRCodeEncoderQRCode');
var QRCodeMatrixUtil = require('./QRCodeMatrixUtil');
var ZXingStringEncoding = require('../../util/ZXingStringEncoding');
var BlockPair = require('./BlockPair');
var WriterException = require('../../WriterException');
...
class QRCodeEncoder {
  ...
  static DEFAULT_BYTE_MODE_ENCODING = CharacterSetECI.CharacterSetECI.UTF8.getName();
  // "ISO-8859-1"
  ...
  static calculateMaskPenalty(matrix) {
    return QRCodeMaskUtil.QRCodeMaskUtil.applyMaskPenaltyRule1(matrix) + QRCodeMaskUtil.QRCodeMaskUtil.applyMaskPenaltyRule2(matrix) + QRCodeMaskUtil.QRCodeMaskUtil.applyMaskPenaltyRule3(matrix) + QRCodeMaskUtil.QRCodeMaskUtil.applyMaskPenaltyRule4(matrix);
  }
  /**
  ...
  static encode(content, ecLevel, hints = null) {
    ...
    const hasEncodingHint = hints !== null && void 0 !== hints.get(EncodeHintType.EncodeHintType.CHARACTER_SET);
    if (hasEncodingHint) {
      encoding = hints.get(EncodeHintType.EncodeHintType.CHARACTER_SET).toString();
    }
    const mode = this.chooseMode(content, encoding);
    const headerBits = new BitArray.BitArray();
    if (mode === QRCodeMode.QRCodeMode.BYTE && (hasEncodingHint || QRCodeEncoder.DEFAULT_BYTE_MODE_ENCODING !== encoding)) {
    ...
    const dataBits = new BitArray.BitArray();
    this.appendBytes(content, mode, dataBits, encoding);
    let version;
    if (hints !== null && void 0 !== hints.get(EncodeHintType.EncodeHintType.QR_VERSION)) {
      const versionNumber = Number.parseInt(hints.get(EncodeHintType.EncodeHintType.QR_VERSION).toString(), 10);
      version = QRCodeVersion.QRCodeVersion.getVersionForNumber(versionNumber);
      const bitsNeeded = this.calculateBitsNeeded(mode, headerBits, dataBits, version);
      if (!this.willFit(bitsNeeded, version, ecLevel)) {
        throw new WriterException.WriterException("Data too big for requested version");
      }
    } else {
      version = this.recommendVersion(ecLevel, mode, headerBits, dataBits);
    }
    const headerAndDataBits = new BitArray.BitArray();
    headerAndDataBits.appendBitArray(headerBits);
    const numLetters = mode === QRCodeMode.QRCodeMode.BYTE ? dataBits.getSizeInBytes() : content.length;
    ...
    const qrCode = new QRCodeEncoderQRCode.QRCodeEncoderQRCode();
    qrCode.setECLevel(ecLevel);
    qrCode.setMode(mode);
    qrCode.setVersion(version);
    const dimension = version.getDimensionForVersion();
    const matrix = new QRCodeByteMatrix.QRCodeByteMatrix(dimension, dimension);
    const maskPattern = this.chooseMaskPattern(finalBits, ecLevel, version, matrix);
    qrCode.setMaskPattern(maskPattern);
    QRCodeMatrixUtil.QRCodeMatrixUtil.buildMatrix(finalBits, ecLevel, version, maskPattern, matrix);
  ...
  static recommendVersion(ecLevel, mode, headerBits, dataBits) {
    const provisionalBitsNeeded = this.calculateBitsNeeded(mode, headerBits, dataBits, QRCodeVersion.QRCodeVersion.getVersionForNumber(1));
  ...
  static chooseMode(content, encoding = null) {
    if (CharacterSetECI.CharacterSetECI.SJIS.getName() === encoding && this.isOnlyDoubleByteKanji(content)) {
      return QRCodeMode.QRCodeMode.KANJI;
    }
    ...
    for (let i = 0, length = content.length; i < length; ++i) {
      ...
      if (QRCodeEncoder.isDigit(c)) {
        ...
        return QRCodeMode.QRCodeMode.BYTE;
      }
    }
    if (hasAlphanumeric) {
      return QRCodeMode.QRCodeMode.ALPHANUMERIC;
    }
    if (hasNumeric) {
      return QRCodeMode.QRCodeMode.NUMERIC;
    }
    return QRCodeMode.QRCodeMode.BYTE;
  }
  static isOnlyDoubleByteKanji(content) {
    let bytes;
    try {
      bytes = ZXingStringEncoding.ZXingStringEncoding.encode(content, CharacterSetECI.CharacterSetECI.SJIS);
    } catch (ignored) {
      return false;
    }
  ...
  static chooseMaskPattern(bits, ecLevel, version, matrix) {
    ...
    for (let maskPattern = 0; maskPattern < QRCodeEncoderQRCode.QRCodeEncoderQRCode.NUM_MASK_PATTERNS; maskPattern++) {
      QRCodeMatrixUtil.QRCodeMatrixUtil.buildMatrix(bits, ecLevel, version, maskPattern, matrix);
  ...
  static chooseVersion(numInputBits, ecLevel) {
    for (let versionNum = 1; versionNum <= 40; versionNum++) {
      const version = QRCodeVersion.QRCodeVersion.getVersionForNumber(versionNum);
      if (QRCodeEncoder.willFit(numInputBits, version, ecLevel)) {
        return version;
      }
    }
    throw new WriterException.WriterException("Data too big");
  }
  /**
  ...
  static terminateBits(numDataBytes, bits) {
    ...
    if (bits.getSize() > capacity) {
      throw new WriterException.WriterException("data bits cannot fit in the QR Code" + bits.getSize() + " > " + capacity);
    }
    ...
    if (bits.getSize() !== capacity) {
      throw new WriterException.WriterException("Bits size does not equal capacity");
    }
  }
  /**
  ...
  static getNumDataBytesAndNumECBytesForBlockID(numTotalBytes, numDataBytes, numRSBlocks, blockID, numDataBytesInBlock, numECBytesInBlock) {
    if (blockID >= numRSBlocks) {
      throw new WriterException.WriterException("Block ID too large");
    }
    ...
    if (numEcBytesInGroup1 !== numEcBytesInGroup2) {
      throw new WriterException.WriterException("EC bytes mismatch");
    }
    if (numRSBlocks !== numRsBlocksInGroup1 + numRsBlocksInGroup2) {
      throw new WriterException.WriterException("RS blocks mismatch");
    }
    if (numTotalBytes !== (numDataBytesInGroup1 + numEcBytesInGroup1) * numRsBlocksInGroup1 + (numDataBytesInGroup2 + numEcBytesInGroup2) * numRsBlocksInGroup2) {
      throw new WriterException.WriterException("Total bytes mismatch");
    }
  ...
  static interleaveWithECBytes(bits, numTotalBytes, numDataBytes, numRSBlocks) {
    if (bits.getSizeInBytes() !== numDataBytes) {
      throw new WriterException.WriterException("Number of bits and data bytes does not match");
    }
    ...
    for (let i = 0; i < numRSBlocks; ++i) {
      ...
      blocks.push(new BlockPair.BlockPair(dataBytes, ecBytes));
      maxNumDataBytes = Math.max(maxNumDataBytes, size);
      maxNumEcBytes = Math.max(maxNumEcBytes, ecBytes.length);
      dataBytesOffset += numDataBytesInBlock[0];
    }
    if (numDataBytes !== dataBytesOffset) {
      throw new WriterException.WriterException("Data bytes does not match offset");
    }
    const result = new BitArray.BitArray();
    ...
    if (numTotalBytes !== result.getSizeInBytes()) {
      throw new WriterException.WriterException("Interleaving error: " + numTotalBytes + " and " + result.getSizeInBytes() + " differ.");
    }
    return result;
  }
  static generateECBytes(dataBytes, numEcBytesInBlock) {
    ...
    new ReedSolomonEncoder.ReedSolomonEncoder(GenericGF.GenericGF.QR_CODE_FIELD_256).encode(toEncode, numEcBytesInBlock);
  ...
  static appendLengthInfo(numLetters, version, mode, bits) {
    ...
    if (numLetters >= 1 << numBits) {
      throw new WriterException.WriterException(numLetters + " is bigger than " + ((1 << numBits) - 1));
    }
  ...
  static appendBytes(content, mode, bits, encoding) {
    switch (mode) {
      case QRCodeMode.QRCodeMode.NUMERIC:
        QRCodeEncoder.appendNumericBytes(content, bits);
        break;
      case QRCodeMode.QRCodeMode.ALPHANUMERIC:
        QRCodeEncoder.appendAlphanumericBytes(content, bits);
        break;
      case QRCodeMode.QRCodeMode.BYTE:
        QRCodeEncoder.append8BitBytes(content, bits, encoding);
        break;
      case QRCodeMode.QRCodeMode.KANJI:
        QRCodeEncoder.appendKanjiBytes(content, bits);
        break;
      default:
        throw new WriterException.WriterException("Invalid mode: " + mode);
    }
  }
  ...
  static appendAlphanumericBytes(content, bits) {
    ...
    while (i < length) {
      ...
      if (code1 === -1) {
        throw new WriterException.WriterException();
      }
      if (i + 1 < length) {
  ...
  static append8BitBytes(content, bits, encoding) {
    let bytes;
    try {
      bytes = ZXingStringEncoding.ZXingStringEncoding.encode(content, encoding);
    } catch (uee) {
      throw new WriterException.WriterException(uee);
    }
  ...
  static appendKanjiBytes(content, bits) {
    ...
    for (let i = 0; i < length; i += 2) {
      ...
      if (subtracted === -1) {
        throw new WriterException.WriterException("Invalid byte sequence");
      }
  ...
  static appendECI(eci, bits) {
    bits.appendBits(QRCodeMode.QRCodeMode.ECI.getBits(), 4);
    bits.appendBits(eci.getValue(), 8);
  }
}

exports.QRCodeEncoder = QRCodeEncoder;
//# sourceMappingURL=QRCodeEncoder.cjs.map
//# sourceMappingURL=QRCodeEncoder.cjs.map