...
var AlignmentPatternFinder = require('./AlignmentPatternFinder');
var FinderPatternFinder = require('./FinderPatternFinder');
...
class Detector {
  ...
  detect(hints) {
    this.resultPointCallback = hints === null || hints === void 0 ? null : (
      /*(ResultPointCallback) */
      hints.get(DecodeHintType.DecodeHintType.NEED_RESULT_POINT_CALLBACK)
    );
    const finder = new FinderPatternFinder.FinderPatternFinder(this.image, this.resultPointCallback);
  ...
  processFinderPatternInfo(info) {
    ...
    if (moduleSize < 1) {
      throw new NotFoundException.NotFoundException("No pattern found in proccess finder.");
    }
    ...
    if (provisionalVersion.getAlignmentPatternCenters().length > 0) {
      ...
      for (let i = 4; i <= 16; i <<= 1) {
        try {
          ...
          if (!(re instanceof NotFoundException.NotFoundException)) {
            throw re;
          }
        }
      }
  ...
  static computeDimension(topLeft, topRight, bottomLeft, moduleSize) {
    const tltrCentersDimension = MathUtils.MathUtils.round(ResultPoint.ResultPoint.distance(topLeft, topRight) / moduleSize);
    const tlblCentersDimension = MathUtils.MathUtils.round(ResultPoint.ResultPoint.distance(topLeft, bottomLeft) / moduleSize);
    ...
    switch (dimension & 3) {
      ...
      case 2:
        ...
        break;
      case 3:
        throw new NotFoundException.NotFoundException("Dimensions could be not found.");
    }
    return dimension;
  }
  /**
  ...
  sizeOfBlackWhiteBlackRun(fromX, fromY, toX, toY) {
    ...
    for (let x = fromX, y = fromY; x !== xLimit; x += xstep) {
      ...
      if (state === 1 === this.image.get(realX, realY)) {
        if (state === 2) {
          return MathUtils.MathUtils.distance(x, y, fromX, fromY);
        }
        state++;
      }
      error += dy;
    ...
    if (state === 2) {
      return MathUtils.MathUtils.distance(toX + xstep, toY, fromX, fromY);
    }
    return NaN;
  }
  /**
  ...
  findAlignmentInRegion(overallEstModuleSize, estAlignmentX, estAlignmentY, allowanceFactor) {
    ...
    if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) {
      throw new NotFoundException.NotFoundException("Alignment top exceeds estimated module size.");
    }
    const alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
    const alignmentAreaBottomY = Math.min(this.image.getHeight() - 1, estAlignmentY + allowance);
    if (alignmentAreaBottomY - alignmentAreaTopY < overallEstModuleSize * 3) {
      throw new NotFoundException.NotFoundException("Alignment bottom exceeds estimated module size.");
    }
    const alignmentFinder = new AlignmentPatternFinder.AlignmentPatternFinder(
      this.image,
      alignmentAreaLeftX,
      alignmentAreaTopY,
