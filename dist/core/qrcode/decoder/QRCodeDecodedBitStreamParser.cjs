...
var BitSource = require('../../common/BitSource');
...
var QRCodeMode = require('./QRCodeMode');
...
class QRCodeDecodedBitStreamParser {
  ...
  static decode(bytes, version, ecLevel, hints) {
    const bits = new BitSource.BitSource(bytes);
    ...
    try {
      ...
      do {
        if (bits.available() < 4) {
          mode = QRCodeMode.QRCodeMode.TERMINATOR;
        } else {
          const modeBits = bits.readBits(4);
          mode = QRCodeMode.QRCodeMode.forBits(modeBits);
        }
        switch (mode) {
          case QRCodeMode.QRCodeMode.TERMINATOR:
            break;
          case QRCodeMode.QRCodeMode.FNC1_FIRST_POSITION:
          case QRCodeMode.QRCodeMode.FNC1_SECOND_POSITION:
            fc1InEffect = true;
            break;
          case QRCodeMode.QRCodeMode.STRUCTURED_APPEND:
            ...
            break;
          case QRCodeMode.QRCodeMode.ECI:
            const value = QRCodeDecodedBitStreamParser.parseECIValue(bits);
            currentCharacterSetECI = CharacterSetECI.CharacterSetECI.getCharacterSetECIByValue(value);
            if (currentCharacterSetECI === null) {
              throw new FormatException.FormatException();
            }
            break;
          case QRCodeMode.QRCodeMode.HANZI:
      ...
      } while (mode !== QRCodeMode.QRCodeMode.TERMINATOR);
    } catch (iae) {
      throw new FormatException.FormatException();
    }
    return new DecoderResult.DecoderResult(
      bytes,
      result.toString(),
  ...
  static decodeHanziSegment(bits, result, count) {
    ...
    try {
      result.append(ZXingStringEncoding.ZXingStringEncoding.decode(buffer, StringUtils.StringUtils.GB2312));
    } catch (ignored) {
      throw new FormatException.FormatException(ignored);
    }
  }
  static decodeKanjiSegment(bits, result, count) {
    ...
    try {
      result.append(ZXingStringEncoding.ZXingStringEncoding.decode(buffer, StringUtils.StringUtils.SHIFT_JIS));
    } catch (ignored) {
      throw new FormatException.FormatException(ignored);
    }
  }
  static decodeByteSegment(bits, result, count, currentCharacterSetECI, byteSegments, hints) {
    ...
    let encoding;
    if (currentCharacterSetECI === null) {
      encoding = StringUtils.StringUtils.guessEncoding(readBytes, hints);
    } else {
      encoding = currentCharacterSetECI.getName();
    }
    try {
      result.append(ZXingStringEncoding.ZXingStringEncoding.decode(readBytes, encoding));
    } catch (ignored) {
      throw new FormatException.FormatException(ignored);
    }
...
exports.QRCodeDecodedBitStreamParser = QRCodeDecodedBitStreamParser;
//# sourceMappingURL=QRCodeDecodedBitStreamParser.cjs.map
//# sourceMappingURL=QRCodeDecodedBitStreamParser.cjs.map