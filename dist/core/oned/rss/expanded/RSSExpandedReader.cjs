...
var AbstractRSSReader = require('../../rss/AbstractRSSReader');
var DataCharacter = require('../../rss/DataCharacter');
var FinderPattern = require('../../rss/FinderPattern');
var RSSUtils = require('../../rss/RSSUtils');
var BitArrayBuilder = require('./BitArrayBuilder');
var AbstractExpandedDecoderComplement = require('./decoders/AbstractExpandedDecoderComplement');
var ExpandedPair = require('./ExpandedPair');
var ExpandedRow = require('./ExpandedRow');

class RSSExpandedReader extends AbstractRSSReader.AbstractRSSReader {
  ...
  decodeRow(rowNumber, row, hints) {
    ...
    try {
      ...
      if (ex instanceof NotFoundException.NotFoundException) ; else {
        throw ex;
      }
    }
  ...
  storeRow(rowNumber) {
    ...
    this.rows.splice(insertPos, 0, new ExpandedRow.ExpandedRow(this.pairs, rowNumber));
    ...
    for (let rowsIndex = rows.length - 1; rowsIndex >= 0; rowsIndex--) {
      ...
      if (r.getPairs().length !== pairs.length) {
        ...
        for (const p of r.getPairs()) {
          if (!pairs.some((otherPair) => ExpandedPair.ExpandedPair.equals(p, otherPair))) {
            allFound = false;
            break;
          }
        }
    ...
    for (const r of rows) {
      ...
      for (const p of pairs) {
        ...
        for (const pp of r.getPairs()) {
          if (ExpandedPair.ExpandedPair.equals(p, pp)) {
            found = true;
            break;
          }
        }
  ...
  static constructResult(pairs) {
    const binary = BitArrayBuilder.BitArrayBuilder.buildBitArray(pairs);
    const decoder = AbstractExpandedDecoderComplement.createAbstractExpandedDecoder(binary);
  ...
  retrieveNextPair(row, previousPairs, rowNumber) {
    ...
    return new ExpandedPair.ExpandedPair(leftChar, rightChar, pattern);
  }
  ...
  parseFoundFinderPattern(row, rowNumber, oddPattern, previousPairs) {
    ...
    const counters = this.getDecodeFinderCounters();
    ZXingSystem.ZXingSystem.arraycopy(counters, 0, counters, 1, counters.length - 1);
    ...
    return new FinderPattern.FinderPattern(value, [start, end], start, end, rowNumber);
  }
  decodeDataCharacter(row, pattern, isOddPattern, leftChar) {
    ...
    let numModules = 17;
    let elementWidth = MathUtils.MathUtils.sum(new Int32Array(counters)) / numModules;
    ...
    return new DataCharacter.DataCharacter(value, checksumPortion);
  }
...
//# sourceMappingURL=RSSExpandedReader.cjs.map
//# sourceMappingURL=RSSExpandedReader.cjs.map