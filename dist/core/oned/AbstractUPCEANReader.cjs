...
class AbstractUPCEANReader extends OneDReader.OneDReader {
  // These two values are critical for determining how permissive the decoding will be.
  // We've arrived at these values through a lot of trial and error. Setting them any higher
  // lets false positives creep in quickly.
  static MAX_AVG_VARIANCE = 0.48;
  static MAX_INDIVIDUAL_VARIANCE = 0.7;
  /**
   * Start/end guard pattern.
   */
  static START_END_PATTERN = Int32Array.from([1, 1, 1]);
  /**
   * Pattern marking the middle of a UPC/EAN pattern, separating the two halves.
   */
  static MIDDLE_PATTERN = Int32Array.from([1, 1, 1, 1, 1]);
  /**
   * end guard pattern.
   */
  static END_PATTERN = Int32Array.from([1, 1, 1, 1, 1, 1]);
  /**
  ...
  */
    ...
    for (let x = rowOffset; x < width; x++) {
      if (row.get(x) !== isWhite) {
        ...
        if (counterPosition === patternLength - 1) {
          if (OneDReader.OneDReader.patternMatchVariance(counters, pattern, AbstractUPCEANReader.MAX_INDIVIDUAL_VARIANCE) < AbstractUPCEANReader.MAX_AVG_VARIANCE) {
  ...
  static decodeDigit(row, counters, rowOffset, patterns) {
    ...
    for (let i = 0; i < max; i++) {
      ...
      let variance = OneDReader.OneDReader.patternMatchVariance(counters, pattern, AbstractUPCEANReader.MAX_INDIVIDUAL_VARIANCE);
...
exports.AbstractUPCEANReader = AbstractUPCEANReader;
//# sourceMappingURL=AbstractUPCEANReader.cjs.map
//# sourceMappingURL=AbstractUPCEANReader.cjs.map