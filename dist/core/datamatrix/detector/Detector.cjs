...
class Detector {
  ...
  constructor(image) {
    this.image = image;
    this.rectangleDetector = new WhiteRectangleDetector.WhiteRectangleDetector(this.image);
  }
  /**
  ...
  detect() {
    ...
    return new DetectorResult.DetectorResult(bits, [topLeft, bottomLeft, bottomRight, topRight]);
  }
  static shiftPoint(point, to, div) {
    let x = (to.getX() - point.getX()) / (div + 1);
    let y = (to.getY() - point.getY()) / (div + 1);
    return new ResultPoint.ResultPoint(point.getX() + x, point.getY() + y);
  }
