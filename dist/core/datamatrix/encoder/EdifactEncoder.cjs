...
class EdifactEncoder {
  ...
  encode(context) {
    ...
    buffer.append(StringUtils.StringUtils.getCharAt(31));
  ...
  encodeChar(c, sb) {
    if (c >= " ".charCodeAt(0) && c <= "?".charCodeAt(0)) {
      ...
      sb.append(StringUtils.StringUtils.getCharAt(c - 64));
    } else {
  ...
  encodeToCodewords(sb) {
    ...
    const cw2 = v >> 8 & 255;
    const cw3 = v & 255;
    const res = new StringBuilder.ZXingStringBuilder();
    res.append(cw1);
    if (len >= 2) {
      res.append(cw2);
    }
...
//# sourceMappingURL=EdifactEncoder.cjs.map
//# sourceMappingURL=EdifactEncoder.cjs.map