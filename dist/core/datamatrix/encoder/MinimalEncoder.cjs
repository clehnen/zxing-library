...
var MinimalECIInput = require('../../common/MinimalECIInput');
...
class MinimalEncoder {
   ...
   */
  static encodeHighLevel(msg, priorityCharset = null, fnc1 = -1, shape = constants.SymbolShapeHint.FORCE_NONE) {
    let macroId = 0;
    if (msg.startsWith(constants.MACRO_05_HEADER) && msg.endsWith(constants.MACRO_TRAILER)) {
      macroId = 5;
      msg = msg.substring(constants.MACRO_05_HEADER.length, msg.length - 2);
    } else if (msg.startsWith(constants.MACRO_06_HEADER) && msg.endsWith(constants.MACRO_TRAILER)) {
      macroId = 6;
      msg = msg.substring(constants.MACRO_06_HEADER.length, msg.length - 2);
    }
  ...
  static getNumberOfC40Words(input, from, c40, characterLength) {
    ...
    for (let i = from; i < input.length(); i++) {
      ...
      if (c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeC40(ci) || !c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeText(ci)) {
        thirdsCount++;
      } else if (!MinimalEncoder.isExtendedASCII(ci, input.getFNC1Character())) {
        thirdsCount += 2;
      } else {
        const asciiValue = ci & 255;
        if (asciiValue >= 128 && (c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeC40(asciiValue - 128) || !c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeText(asciiValue - 128))) {
          thirdsCount += 3;
        } else {
          thirdsCount += 4;
        }
  ...
  static addEdges(input, edges, from, previous) {
    ...
    if (previous === null || previous.getEndMode() !== 4 /* EDF */) {
      if (DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(ch) && input.haveNCharacters(from, 2) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(input.charAt(from + 1))) {
      ...
      if (input.haveNCharacters(from, 3) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeX12(input.charAt(from)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeX12(input.charAt(from + 1)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeX12(input.charAt(from + 2))) {
    ...
    for (i = 0; i < 3; i++) {
      const pos = from + i;
      if (input.haveNCharacters(pos, 1) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeEDIFACT(input.charAt(pos))) {
        this.addEdge(edges, new Edge(input, 4 /* EDF */, from, i + 1, previous));
      } else {
        break;
      }
    }
    if (i === 3 && input.haveNCharacters(from, 4) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeEDIFACT(input.charAt(from + 3))) {
  ...
  static encodeMinimally(input) {
    ...
    let minimalJ = -1;
    let minimalSize = ZXingInteger.ZXingInteger.MAX_VALUE;
    for (let j = 0; j < 6; j++) {
...
class Edge {
  ...
  getLastASCII() {
    ...
    if (length - from === 2) {
      ...
      if (DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 1))) {
        return 1;
      }
      return 2;
    }
    if (length - from === 3) {
      if (DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 1)) && !MinimalEncoder.isExtendedASCII(
        this.input.charAt(from + 2),
        this.input.getFNC1Character()
      )) {
        return 2;
      }
      if (DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 1)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 2)) && !MinimalEncoder.isExtendedASCII(
        this.input.charAt(from),
        this.input.getFNC1Character()
      )) {
        return 2;
    ...
    if (DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 1)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 2)) && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isDigit(this.input.charAt(from + 3))) {
      return 2;
    }
    return 0;
  }
  ...
  getMinSymbolSize(minimum) {
    switch (this.input.getShapeHint()) {
      case constants.SymbolShapeHint.FORCE_SQUARE:
        ...
        break;
      case constants.SymbolShapeHint.FORCE_RECTANGLE:
  ...
  getC40Words(c40, fnc1) {
    ...
    for (let i = 0; i < this.characterLength; i++) {
      ...
      if (c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeC40(ci) || !c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeText(ci)) {
        ...
        if (c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeC40(asciiValue) || !c40 && DataMatrixHighLevelEncoder.DataMatrixHighLevelEncoder.isNativeText(asciiValue)) {
          c40Values.push(1);
          c40Values.push(30);
...
class Input extends MinimalECIInput.MinimalECIInput {
...
//# sourceMappingURL=MinimalEncoder.cjs.map
//# sourceMappingURL=MinimalEncoder.cjs.map