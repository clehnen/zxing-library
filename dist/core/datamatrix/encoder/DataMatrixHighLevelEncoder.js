...
import { SymbolShapeHint, MACRO_05_HEADER, MACRO_TRAILER, MACRO_05, MACRO_06_HEADER, MACRO_06, ASCII_ENCODATION, BASE256_ENCODATION, EDIFACT_ENCODATION, PAD, X12_ENCODATION, TEXT_ENCODATION, C40_ENCODATION } from './constants';
...
class DataMatrixHighLevelEncoder {
   ...
   */
  static encodeHighLevel(msg, shape = SymbolShapeHint.FORCE_NONE, minSize = null, maxSize = null, forceC40 = false) {
    ...
    let encodingMode = ASCII_ENCODATION;
    if (forceC40) {
  ...
  static lookAheadTestIntern(msg, startpos, currentMode) {
    ...
    while (true) {
      if (startpos + charsProcessed === msg.length) {
        ZXingArrays.fill(mins, 0);
        ZXingArrays.fill(intCharCounts, 0);
        const min = this.findMinimums(
          charCounts,
          intCharCounts,
          ZXingInteger.MAX_VALUE,
          mins
        );
      ...
      if (charsProcessed >= 4) {
        ZXingArrays.fill(mins, 0);
        ZXingArrays.fill(intCharCounts, 0);
        this.findMinimums(charCounts, intCharCounts, ZXingInteger.MAX_VALUE, mins);
        if (intCharCounts[ASCII_ENCODATION] < this.min(
        ...
        if (intCharCounts[BASE256_ENCODATION] < intCharCounts[ASCII_ENCODATION] || intCharCounts[BASE256_ENCODATION] + 1 < this.min(
          intCharCounts[C40_ENCODATION],
          intCharCounts[TEXT_ENCODATION],
          intCharCounts[X12_ENCODATION],
          intCharCounts[EDIFACT_ENCODATION]
        ...
        if (intCharCounts[EDIFACT_ENCODATION] + 1 < this.min(
          intCharCounts[BASE256_ENCODATION],
          intCharCounts[C40_ENCODATION],
          intCharCounts[TEXT_ENCODATION],
          intCharCounts[X12_ENCODATION],
        ...
        if (intCharCounts[TEXT_ENCODATION] + 1 < this.min(
          intCharCounts[BASE256_ENCODATION],
          intCharCounts[C40_ENCODATION],
          intCharCounts[EDIFACT_ENCODATION],
          intCharCounts[X12_ENCODATION],
        ...
        if (intCharCounts[X12_ENCODATION] + 1 < this.min(
          intCharCounts[BASE256_ENCODATION],
          intCharCounts[C40_ENCODATION],
          intCharCounts[EDIFACT_ENCODATION],
          intCharCounts[TEXT_ENCODATION],
        ...
        if (intCharCounts[C40_ENCODATION] + 1 < this.min(
          intCharCounts[ASCII_ENCODATION],
          intCharCounts[BASE256_ENCODATION],
          intCharCounts[EDIFACT_ENCODATION],
          intCharCounts[TEXT_ENCODATION]
  ...
  static illegalCharacter(singleCharacter) {
    let hex = ZXingInteger.toHexString(singleCharacter.charCodeAt(0));
...
export { DataMatrixHighLevelEncoder };
//# sourceMappingURL=DataMatrixHighLevelEncoder.js.map
//# sourceMappingURL=DataMatrixHighLevelEncoder.js.map