...
class Result {
  ...
  // }
  constructor(text, rawBytes, numBits = rawBytes == null ? 0 : 8 * rawBytes.length, resultPoints, format, timestamp = ZXingSystem.ZXingSystem.currentTimeMillis()) {
    this.text = text;
    ...
    if (void 0 === timestamp || null === timestamp) {
      this.timestamp = ZXingSystem.ZXingSystem.currentTimeMillis();
    } else {
      this.timestamp = timestamp;
    }
  }
  ...
  addResultPoints(newPoints) {
    ...
    if (oldPoints === null) {
      ...
      ZXingSystem.ZXingSystem.arraycopy(oldPoints, 0, allPoints, 0, oldPoints.length);
      ZXingSystem.ZXingSystem.arraycopy(newPoints, 0, allPoints, oldPoints.length, newPoints.length);
