 ...
 */
export /*final*/ class PDF417DecodedBitStreamParser {
  ...
  static decodeMacroBlock(codewords: Int32Array, codeIndex: int, resultMetadata: PDF417ResultMetadata): int {
    ...
    let segmentIndexArray: Int32Array = new Int32Array(PDF417DecodedBitStreamParser.NUMBER_OF_SEQUENCE_CODEWORDS);
    for (let i /*int*/ = 0; i < PDF417DecodedBitStreamParser.NUMBER_OF_SEQUENCE_CODEWORDS; i++, codeIndex++) {
      segmentIndexArray[i] = codewords[codeIndex];
    }
    resultMetadata.setSegmentIndex(ZXingInteger.parseInt(PDF417DecodedBitStreamParser.decodeBase900toBase10(segmentIndexArray,
      PDF417DecodedBitStreamParser.NUMBER_OF_SEQUENCE_CODEWORDS)));

    let fileId: ZXingStringBuilder = new ZXingStringBuilder();
        ...
        case PDF417DecodedBitStreamParser.BEGIN_MACRO_PDF417_OPTIONAL_FIELD:
          ...
          switch (codewords[codeIndex]) {
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_FILE_NAME:
              let fileName: ZXingStringBuilder = new ZXingStringBuilder();
            ...
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_SENDER:
              let sender: ZXingStringBuilder = new ZXingStringBuilder();
            ...
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_ADDRESSEE:
              let addressee: ZXingStringBuilder = new ZXingStringBuilder();
            ...
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_SEGMENT_COUNT:
              let segmentCount: ZXingStringBuilder = new ZXingStringBuilder();
              codeIndex = PDF417DecodedBitStreamParser.numericCompaction(codewords, codeIndex + 1, segmentCount);
              resultMetadata.setSegmentCount(ZXingInteger.parseInt(segmentCount.toString()));
              break;
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_TIME_STAMP:
              let timestamp: ZXingStringBuilder = new ZXingStringBuilder();
            ...
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_CHECKSUM:
              let checksum: ZXingStringBuilder = new ZXingStringBuilder();
              codeIndex = PDF417DecodedBitStreamParser.numericCompaction(codewords, codeIndex + 1, checksum);
              resultMetadata.setChecksum(ZXingInteger.parseInt(checksum.toString()));
              break;
            case PDF417DecodedBitStreamParser.MACRO_PDF417_OPTIONAL_FIELD_FILE_SIZE:
              let fileSize: ZXingStringBuilder = new ZXingStringBuilder();
    ...
    if (optionalFieldsStart !== -1) {
      ...
      resultMetadata.setOptionalData(ZXingArrays.copyOfRange(codewords, optionalFieldsStart, optionalFieldsStart + optionalFieldsLength));
    }
   ...
   */
  private static textCompaction(codewords: Int32Array, codeIndex: int, result: ZXingStringBuilder): int {
  ...
  private static decodeTextCompaction(textCompactionData: Int32Array,
    ...
    length: int,
    result: ZXingStringBuilder): void {
  ...
  private static /*int*/ byteCompaction(mode: int,
    ...
    result.append(ZXingStringEncoding.decode(decodedBytes.toByteArray(), encoding));
    return codeIndex;
  }
   ...
   * @throws FormatException
   */
  private static numericCompaction(codewords: Int32Array, codeIndex: number /*int*/, result: ZXingStringBuilder): int {
    let count: int = 0;
    let end: boolean = false;

    let numericCodewords: Int32Array = new Int32Array(PDF417DecodedBitStreamParser.MAX_NUMERIC_CODEWORDS);
