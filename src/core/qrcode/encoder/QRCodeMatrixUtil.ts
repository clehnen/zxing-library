...
export class QRCodeMatrixUtil {
    ...
    public static clearMatrix(matrix: QRCodeByteMatrix): void {
    ...
    public static buildMatrix(dataBits: BitArray,
        ...
        matrix: QRCodeByteMatrix): void /*throws WriterException*/ {
    ...
    public static embedBasicPatterns(version: QRCodeVersion, matrix: QRCodeByteMatrix): void /*throws WriterException*/ {
    ...
    public static embedTypeInfo(ecLevel: QRCodeDecoderErrorCorrectionLevel, maskPattern: number /*int*/, matrix: QRCodeByteMatrix): void {
        ...
        for (let i = 0, size = typeInfoBits.getSize(); i < size; ++i) {
            ...
            const coordinates: Int32Array = QRCodeMatrixUtil.TYPE_INFO_COORDINATES[i];
    ...
    public static maybeEmbedVersionInfo(version: QRCodeVersion, matrix: QRCodeByteMatrix): void /*throws WriterException*/ {
    ...
    public static embedDataBits(dataBits: BitArray, maskPattern: number /*int*/, matrix: QRCodeByteMatrix): void {
        let bitIndex = 0;
        let direction = -1;
    ...
    public static makeTypeInfoBits(ecLevel: QRCodeDecoderErrorCorrectionLevel, maskPattern: number /*int*/, bits: BitArray): void {
    ...
    public static makeVersionInfoBits(version: QRCodeVersion, bits: BitArray): void /*throws WriterException*/ {
    ...
    private static embedTimingPatterns(matrix: QRCodeByteMatrix): void {
    ...
    private static embedDarkDotAtLeftBottomCorner(matrix: QRCodeByteMatrix): void /*throws WriterException*/ {
    ...
    private static embedPositionAdjustmentPattern(xStart: number /*int*/, yStart: number /*int*/, matrix: QRCodeByteMatrix): void {
        for (let y = 0; y < 5; ++y) {
            const patternY: Int32Array = QRCodeMatrixUtil.POSITION_ADJUSTMENT_PATTERN[y];
            for (let x = 0; x < 5; ++x) {
    ...
    private static embedPositionDetectionPattern(xStart: number /*int*/, yStart: number /*int*/, matrix: QRCodeByteMatrix): void {
        for (let y = 0; y < 7; ++y) {
            const patternY: Int32Array = QRCodeMatrixUtil.POSITION_DETECTION_PATTERN[y];
            for (let x = 0; x < 7; ++x) {
                matrix.setNumber(xStart + x, yStart + y, patternY[x]);
            }
        }
    ...
    private static embedPositionDetectionPatternsAndSeparators(matrix: QRCodeByteMatrix): void /*throws WriterException*/ {
        ...
        // Right top corner.
        QRCodeMatrixUtil.embedHorizontalSeparationPattern(matrix.getWidth() - hspWidth,
            hspWidth - 1, matrix);
        // Left bottom corner.
        ...
        // Left bottom corner.
        QRCodeMatrixUtil.embedVerticalSeparationPattern(vspSize, matrix.getHeight() - vspSize,
            matrix);
    }

    // Embed position adjustment patterns if need be.
    private static maybeEmbedPositionAdjustmentPatterns(version: QRCodeVersion, matrix: QRCodeByteMatrix): void {
        if (version.getVersionNumber() < 2) {  // The patterns appear if version >= 2
            return;
        }
        const index = version.getVersionNumber() - 1;
        const coordinates: Int32Array = QRCodeMatrixUtil.POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[index];
