 ...
 */
export /*public final*/ class AztecEncoder {
  ...
  public static encode(data: Uint8Array, minECCPercent: int, userSpecifiedLayers: int): AztecCode {
    // High-level encode
    let bits: BitArray = new AztecHighLevelEncoder(data).encode();

    // stuff bits and choose symbol size
    let eccBits: int = ZXingInteger.truncDivision((bits.getSize() * minECCPercent), 100) + 11;
    ...
    let messageBits: BitArray = AztecEncoder.generateCheckWords(stuffedBits, totalBitsInLayer, wordSize);

    // generate mode message
    let messageSizeInWords: int = stuffedBits.getSize() / wordSize;
    let modeMessage: BitArray = AztecEncoder.generateModeMessage(compact, layers, messageSizeInWords);
    ...
    if (compact) {
      ...
      let origCenter: int = ZXingInteger.truncDivision(baseMatrixSize, 2);
      let center: int = ZXingInteger.truncDivision(matrixSize, 2);
      for (let i /*int*/ = 0; i < origCenter; i++) {
        let newOffset: int = i + ZXingInteger.truncDivision(i, 15);
    ...
    if (compact) {
      ...
      for (let i /*int*/ = 0, j = 0; i < ZXingInteger.truncDivision(baseMatrixSize, 2) - 1; i += 15, j += 16) {
        for (let k /*int*/ = ZXingInteger.truncDivision(matrixSize, 2) & 1; k < matrixSize; k += 2) {
  ...
  private static drawModeMessage(matrix: BitMatrix, compact: boolean, matrixSize: int, modeMessage: BitArray): void {
    ...
    if (compact) {
      ...
      for (let i /*int*/ = 0; i < 10; i++) {
        let offset: int = center - 5 + i + ZXingInteger.truncDivision(i, 5);
        if (modeMessage.get(i)) {
  ...
  private static generateCheckWords(bitArray: BitArray, totalBits: int, wordSize: int): BitArray {
    ...
    let rs: ReedSolomonEncoder = new ReedSolomonEncoder(AztecEncoder.getGF(wordSize));
    let totalWords: int = ZXingInteger.truncDivision(totalBits, wordSize);
    let messageWords: Int32Array = AztecEncoder.bitsToWords(bitArray, wordSize, totalWords);
