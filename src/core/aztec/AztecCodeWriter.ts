 ...
 */
export /*public final*/ class AztecCodeWriter implements Writer {
  ...
  public encodeWithHints(contents: string, format: BarcodeFormat, width: int, height: int, hints: Map<EncodeHintType, any>): BitMatrix {
    let charset: ZXingCharset = ZXingStandardCharsets.ISO_8859_1;
    let eccPercent: int = AztecEncoder.DEFAULT_EC_PERCENT;
    let layers: int = AztecEncoder.DEFAULT_AZTEC_LAYERS;
    if (hints != null) {
  ...
  private static encodeLayers(contents: string, format: BarcodeFormat,
    ...
    charset: ZXingCharset, eccPercent: int, layers: int): BitMatrix {
    if (format !== BarcodeFormat.AZTEC) {
      throw new IllegalArgumentException('Can only encode AZTEC, but got ' + format);
    }
    let aztec: AztecCode = AztecEncoder.encode(StringUtils.getBytes(contents, charset), eccPercent, layers);
