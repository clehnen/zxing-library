...
export class AztecDetector {
    ...
    private getRotation(sides: Int32Array, length: number): number {
        ...
        for (let shift = 0; shift < 4; shift++) {
            if (ZXingInteger.bitCount(cornerBits ^ this.EXPECTED_CORNER_BITS[shift]) <= 2) {
                return shift;
            }
        }
        throw new NotFoundException();
     ...
     */
    private getBullsEyeCorners(pCenter: AztecPoint): ResultPoint[] {
     ...
     */
    private getMatrixCenter(): AztecPoint {
     ...
     */
    private isWhiteOrBlackRectangle(p1: AztecPoint,
        p2: AztecPoint,
        p3: AztecPoint,
        p4: AztecPoint): boolean {
     ...
     */
    private getColor(p1: AztecPoint, p2: AztecPoint): number {
     ...
     */
    private getFirstDifferent(init: AztecPoint, color: boolean, dx: number, dy: number): AztecPoint {
        let x = init.getX() + dx;
        let y = init.getY() + dy;
    ...
    private distancePoint(a: AztecPoint, b: AztecPoint): number {
    ...
    private getDimension(): number {
        ...
        return 4 * this.nbLayers + 2 * (ZXingInteger.truncDivision((this.nbLayers - 4), 8) + 1) + 15;
    }
