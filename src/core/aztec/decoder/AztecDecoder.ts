...
export class AztecDecoder {
        ...
        while (index < endIndex) {
            if (shiftTable === Table.BINARY) {
                ...
                for (let charCount = 0; charCount < length; charCount++) {
                    ...
                    const code: int = AztecDecoder.readCode(correctedBits, index, 8);
    ...
    private extractBits(matrix: BitMatrix): boolean[] {
        ...
        if (compact) {
            ...
            let matrixSize = baseMatrixSize + 1 + 2 * ZXingInteger.truncDivision((ZXingInteger.truncDivision(baseMatrixSize, 2) - 1), 15);
