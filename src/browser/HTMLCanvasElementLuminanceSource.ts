...
export class HTMLCanvasElementLuminanceSource extends LuminanceSource {
    ...
    public constructor(private canvas: HTMLCanvasElement, doAutoInvert: boolean = false, reuseBuffer?: Uint8ClampedArray) {
      super(canvas.width, canvas.height);
      this.buffer = HTMLCanvasElementLuminanceSource.makeBufferFromCanvasImageData(canvas, doAutoInvert, reuseBuffer);
    }

    private static makeBufferFromCanvasImageData(canvas: HTMLCanvasElement, doAutoInvert: boolean = false, reuseBuffer?: Uint8ClampedArray): Uint8ClampedArray {
    ...
    private static toGrayscaleBuffer(imageBuffer: Uint8ClampedArray, width: number, height: number, doAutoInvert: boolean = false, reuseBuffer?: Uint8ClampedArray): Uint8ClampedArray {
      ...
      const invertMask = (!HTMLCanvasElementLuminanceSource.FRAME_INDEX && doAutoInvert) ? 0xFF : 0;
