...
describe('MicroQRDataMask', () => {
    ...
    it('mask 0: (i+j) mod 2 == 0', () => {
        ...
        expect(bits.get(0, 0)).toBe(true);  // (i=0,j=0): (0+0)%2=0 → flipped
        expect(bits.get(1, 0)).toBe(false); // (i=0,j=1): (0+1)%2=1 → not flipped
        expect(bits.get(0, 1)).toBe(false); // (i=1,j=0): (1+0)%2=1 → not flipped
        expect(bits.get(1, 1)).toBe(true);  // (i=1,j=1): (1+1)%2=0 → flipped
        expect(bits.get(2, 0)).toBe(true);  // (i=0,j=2): (0+2)%2=0 → flipped
    });
    ...
    it('mask 1: i mod 2 == 0', () => {
        ...
        // Row 0 (i=0): all flipped
        expect(bits.get(0, 0)).toBe(true);
        expect(bits.get(5, 0)).toBe(true);
        // Row 1 (i=1): none flipped
        expect(bits.get(0, 1)).toBe(false);
        expect(bits.get(5, 1)).toBe(false);
        // Row 2 (i=2): all flipped
    ...
    it('mask 2: j mod 3 == 0', () => {
        ...
        // Col 0 (j=0): flipped
        expect(bits.get(0, 0)).toBe(true);
        expect(bits.get(0, 5)).toBe(true);
        // Col 1 (j=1): not flipped
        expect(bits.get(1, 0)).toBe(false);
        // Col 3 (j=3): flipped
        expect(bits.get(3, 0)).toBe(true);
        // Col 2 (j=2): not flipped
        expect(bits.get(2, 0)).toBe(false);
    });

    it('mask 3: (i+j) mod 3 == 0', () => {
        const mask = MicroQRDataMask.forIndex(3);
        const dim = 11;
        const bits = new BitMatrix(dim);
        mask.unmaskBitMatrix(bits, dim);

        // (i=0,j=0): 0 mod 3 = 0 → flipped
        expect(bits.get(0, 0)).toBe(true);
        // (i=0,j=1): 1 mod 3 ≠ 0 → not flipped
        expect(bits.get(1, 0)).toBe(false);
        // (i=0,j=3): 3 mod 3 = 0 → flipped
        expect(bits.get(3, 0)).toBe(true);
        // (i=1,j=2): 3 mod 3 = 0 → flipped
        expect(bits.get(2, 1)).toBe(true);
    });
